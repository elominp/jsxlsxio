...
static napi_value xlsxiowrite_add_cell_string_wrapper(napi_env env,
  ...
  ASSERT(napi_get_value_string_utf8(env, argv[1], value, BUFFER_SIZE,
                                    &value_length) == napi_ok);
...
napi_value create_xlsxiowrite_add_cell_string_wrapper(napi_env env) {
  ...
  ASSERT(
      napi_create_function(env, "xlsxiowrite_add_cell_string", NAPI_AUTO_LENGTH,
...
napi_status set_xlsxiowrite_add_cell_string_wrapper(napi_env env,
                                                    napi_value result) {
  return napi_set_named_property(
      env, result, "xlsxiowrite_add_cell_string",
